// v9: apply db/migrations/0013_v9-ems-plans.sql (ems_plans — Contract A).
// Additive CREATE TABLE/INDEX only — safe against the remote production TiDB.
//   npx tsx scripts/apply-v9-ems-plans-migration.ts [path-to-sql]
...
async function main() {
  const db = getDb();
  const file = process.argv[2] ?? "db/migrations/0013_v9-ems-plans.sql";
